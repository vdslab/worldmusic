...
const aboutFeatureAndData = () => {
  return (
    <div>
      ...
      <p>
        0.0から1.0までの値で、トラックがアコースティックかどうかを示す。
        1.0はトラックがアコースティックであるという評価が高いことを表す。
      </p>
      <li><h2 className="title is-5">danceabilityとは？</h2></li>
      <p>
        テンポ、リズムの安定性、ビートの強さ、全体的な規則性などの音楽要素の組み合わせに基づき、
        トラックがダンスにどの程度適しているかを示す。0.0は最も踊りやすくなく、1.0は最も踊りやすい。
      </p>
      <li><h2 className="title is-5">energyとは？</h2></li>
      <p>
        激しさと活発さを知覚できる程度の数字で表し、0.0から1.0までの値で計測する。
        通常、エネルギッシュなトラックは早く、大きく、ノイズが多いと感じる。
        例えば、デスメタルはエネルギーが高いのに対し、バッハの前奏曲は低くなっている。
        知覚できると判断される要素には、ダイナミックレンジ、音の大きさ、音色、出だし（頭子音）、
        および一般的なエントロピーが含まれる。
      </p>
      <li><h2 className="title is-5">instrumentalnessとは？</h2></li>
      <p>
        トラックにボーカルが含まれていないかどうかを予測する。
        なお、「Ooh」と「aah」の音は楽器として取り扱う。ラップやスポークンワード（喋り言葉）のトラックは
        明らかに「ボーカル」である。
        楽器性の値が1.0に近いほど、トラックにボーカルコンテンツが含まれていない可能性が高い。
        0.5を超える値は楽器性の高いトラックを表すが、値が1.0に近づくにつれ、評価が高い（ボーカルを含まない程度）ということを示す。
      </p>
      <li><h2 className="title is-5">livenessとは？</h2></li>
      <p>
        レコードのなかに聴衆の存在がどれくらいあるのかを検出する。値が高いほど、そのトラックがライブで演奏された可能性が高い。
        値が0.8を超す場合は、そのトラックがライブ（生演奏）である可能性が高いことを示す。
      </p>
      <li><h2 className="title is-5">loudnessとは？</h2></li>
      <p>
        音の強さ・大きさ、トラックの全体の音の強さ・大きさを示すデシベル数（dB）。
        値はトラック全体の平均値であり、相関するトラック（同じような感じのトラック）の音の強さ・大きさを比較するときに役立つ。
        主に物理的な強さ（大きさ）に心理的な相関をもたらす音の品質を指している。
        値は一般的には-60から0dbまでの範囲で示される。
      </p>
      ...
      <p>
        ...
        （トークショー、オーディオブック、詩等）に似ているような録音であるほど、値は 1.0 に近づく。 
        0.66を超える値は、完全に話し言葉でできているトラックであろうことを示す。
        0.33から0.66までの値は、セクションまたはレイヤーのいずれかで音楽とスピーチの両方を含む
        可能性のあるトラック（ラップ音楽等の場合も含む）を表す。
        0.33未満の値であれば、そのトラックはほとんど音楽であり、言葉が含まれていないことの可能性が高くなる。
      </p>
      <li><h2 className="title is-5">tempoとは？</h2></li>
      <p>
        トラック全体で見込まれる毎分時のビート（BPM）。
        音楽用語では、テンポは特定の曲の速度またはペースを指し、ビートの持続時間の平均から導き出されるものである。
      </p>
      <li><h2 className="title is-5">time_signatureとは？</h2></li>
      <p>曲の長さを示す。</p>
      <li><h2 className="title is-5">valenceとは？</h2></li>
      <p>
        0.0から1.0までの値を用いてそのトラックの音楽的なポジティブさ（陽気さ）を示す。
        高いほどトラックはよりポジティブに聞こえ（例：幸せ、陽気、陶酔）、

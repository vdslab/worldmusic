  ...
  try {
    if (features.includes(feature)) {
      const result = await selectRows(
        `SELECT SUM ( Ranking.stream * Music.${feature} ) / SUM ( Ranking.stream) AS value , Ranking.countryid  FROM Music INNER JOIN Ranking ON Music.musicid=Ranking.musicid JOIN Country ON Ranking.countryid = Country.countryid WHERE Ranking.startday BETWEEN $1::text AND $2::text AND Country.region = $3::text GROUP BY Ranking.countryid`,
        [s, e, regionId]
      );
